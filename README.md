...
## Features
- **SIMD-accelerated:** Uses `frizbee` for high-performance fuzzy matching.
...
- **High Performance:** Uses `jemalloc` for efficient memory management and background threads for non-blocking I/O and searching.
...
1. **Asynchronous Reading:** Reads stdin in 5000-line chunks in a background thread to keep the UI responsive.
2. **Search Narrowing:** If the new query starts with the previous query, it only searches within the previous results.
3. **Jemalloc:** Uses `tikv-jemallocator` for better performance under high allocation churn.
4. **Direct TTY Access:** Opens `/dev/tty` for input/output to allow usage in shell pipelines where `stdin` is already consumed.
